...
Finaly, `ggplot2 3.0.0` got released last week with the support of `tidy eval`, so I thought it was time to write about it!
...
The common way of using `ggplot2` functions, and the first example most people try is using Non standard Evaluation (`NSE`), by passing column names unquoted. For instance if you want to plot `tip_percent` versus `total_bill` you could simply write the following chunck of code.
...
This version could be helpful when you want pass variables to `ggplot` function. For instance, you can have handle the followinh scenario, where you have pre defined variables to use. It will work fine and give the same result.

```{raes_string_var, eval = FALSE}
...
Then you can either pass column names directly or the variabl names.

```{r, eval = FALSE}
## pass column names directly
plot_points(x = "total_bill", y = "tip_percent", alpha = 0.5)

## pass variable names
plot_points(x = x_var, y = y_var, alpha = alpha_var)
...
plot_points(x = "total_bill", y = "tip", alpha = 0.5)
```
...
## invalid way
plot_points2(x = total_bill, y = tip_percent, alpha = 0.5)
```
...
plot_points_tidyeval_01(x = total_bill, y = tip_percent, alpha = 0.5)
```

You might still see this as a simple use case that could be handled by `aes_string()`. But there are more things yiu can do., like variable facets.
