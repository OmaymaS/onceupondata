...
Another way is using `aes_string()` instead of `aes` and passing column names quoted as follows, which will give the same result.
...
This version could be helpful when you want to pass variables to `ggplot2` function. For instance, you can have handle the following scenario, when you have pre-defined variables to use, or if you take the variable names from a user in a an interface. The following examples will work fine and give the same result.
...
And you can reuse the function to create similar plots with different variables. For instance `tip` on the y-exis instead of `tip_percent`
...
So the previous options are good and valid in many cases, but there are some limitations. For instance, you cannot create a function and pass column names unquoted as follows. 
