...
In this competition, it was required to build a model that’s *"capable of detecting different types of toxicity like threats, obscenity, insults, and identity-based hate"*. The dataset includes thousands of comments from **Wikipedia’s talk page edits** and each comment can more than one tag.
...
As we know the lengths of the comments are not equal, so we need to pad them with zeros to unify their length. We can decide the maximum length based on the data we have. 
...
since `keras` models take matrices, we will select the target columns and convert to matrix as follows:
...
- the accuracy is not bad given the simple model we built. So there' could be 's a room for higher accuracy with more sophisticated models.
...
plot(history)+
  theme_minimal()+
  ggtitle("Loss and Accurace Curves")
```
...
This simple model could get around **0.96 on the Private Score**, which is not bad but still far from the top competitors. It is just a good start, but there are other models that would perform better. In the next post we will see  types of networks.